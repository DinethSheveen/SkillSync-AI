			...
			<header className="p-8 text-white" style={{ backgroundColor: color }}>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm ">
					...
					{data.personalInfo?.github && (
						<a target="_blank" href={data.personalInfo?.github} className="flex items-center gap-2">
							<LuGithub className="size-4" />
							<span className="break-all text-xs">{data.personalInfo.github.split("https://")[1] ? data.personalInfo.github.split("https://")[1] : data.personalInfo.github}</span>
						</a>
					)}
				</div>
			</header>
					...
					<section className="mb-8">
						...
						<div className="space-y-6">
							{data.projects.map((p, index) => (
								<div key={index} className="relative pl-6 border-l border-gray-200" style={{borderLeftColor: color}}>
